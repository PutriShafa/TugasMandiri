...
@section('konten')
        ...
        <div class="row">
            ...
            <div class="col-5 align-self-center">
                <div class="customize-input float-right">
                    <a class="btn btn-primary" href="{{ URL::to('tambah_user') }}">Tambah User</a>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <table id="zero_config" class="table table-striped table-bordered no-wrap">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($modul as $item)
                                        @php
                                            if ($item->id_profile == '1') {
                                                $profil = 'Super Administrator';
                                            } elseif ($item->id_profile == '2') {
                                                $instansi = Instansi::find($item->id_instansi)->instansi;
                                                $profil = 'Administrator';
                                            } elseif ($item->id_profile == '3') {
                                                $loket = Loket::find($item->id_loket);
                                                $instansi = Instansi::find($item->id_instansi)->instansi;
                                                $profil = "Loket ($loket->nama_loket)";
                                            } else {
                                                $instansi = 'Pengguna';
                                                $profil = 'User';
                                            }
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $item->email }}</td>
                                            <td>
                                                <form method="POST" action="{{ URL::to('edit_user') }}">
                                                    @csrf
    ...
    <div id="hapus" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <form method="post" action="{{ URL::to('hapus_user') }}">
            @csrf
